//the fixtures() function produces a week's game fixtures depending on which
//week the user wants to see, and displays them on cards
function fixtures() {
  //these two dictionaries will contain a list of home team tables and away team
  //information
  var FixtureTableHome = {};
  var FixtureTableAway = {};
  //this takes data from the form that the user inputs to select the matchweek
  var data = document.getElementById("MatchdayForm").value;

console.log(data)
  // This makes the GET request for the fixtures, depending on what Matchweek
  // the user wants their fixture information for
  $.ajax({
...
// this function handleData then parses the JSON data and appends the relevant
//information to the empty cards on the page
...
//the function HistoricalChart() draws a c3 chart that contains the team's positions
// up to the present week. it updates live
function HistoricalChart() {
  //Matchweek sets what week the chart is drawn up to
  //count sets the number of teams
  var Matchweek= 23
  var count = 20
  //the requests array contains all of the GET request URL
  var requests = new Array()

  //This generates the array of GET requests
  ...
  //which is a nameless function with error and files input
    for (i=1; i<Matchweek; i++) {
    q.defer(requests[i].get);
  }
    q.awaitAll(function(error, files) {
    if (error) throw error;
    //The for loop converts the retrieved files to a JSON called Matchdays, which is
  ...
  //the findTeamEntry is used to select the array that contains a team's position for a particular
  //matchweek
  ...
  // This section then makes the table in the required format for c3.js
  ...
  //This draws the chart I want to the element with ID #HistoricalChart,in the main
  //HTML page
...
//this calls the function above to finally draw the chart
